...
const ALLOWED_CHANNELS = new Set<string>(
  DONATION_PAYMENT_CHANNELS.map((item) => item.value),
);
...
function getMethodFromChannel(channel: string) {
  if (
    channel === "mpesa" ||
...
function isTenantHostname(hostname: string) {
  const rootDomain = process.env.NEXT_PUBLIC_ROOT_DOMAIN || "mpangi-church.app";

  const normalized = hostname.split(":")[0].toLowerCase();

  return (
    normalized.endsWith(`.${rootDomain}`) && normalized !== `www.${rootDomain}`
  );
}
...
function createConfirmationPath(
  ...
  const basePath = isTenantHostname(hostname)
    ? "/don/success"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const method = getMethodFromChannel(channel);

    if (method === "mobile_money" && !donorPhone) {
      return NextResponse.json(
        {
          error: "Le téléphone du donateur est requis pour Mobile Money.",
        },
        { status: 400 },
      );
    }
    ...
    const allowedCurrencies = Array.isArray(church.donation_allowed_currencies)
    ...
    const mobileConfig = {
      mpesa: {
        enabled: church.donation_mpesa_enabled,
        number: church.donation_mpesa_number,
        operator: "M-Pesa",
        accountName: church.donation_mpesa_name,
      },
      airtel_money: {
        enabled: church.donation_airtel_enabled,
        number: church.donation_airtel_number,
        operator: "Airtel Money",
        accountName: church.donation_airtel_name,
      },
      orange_money: {
        enabled: church.donation_orange_enabled,
        number: church.donation_orange_number,
        operator: "Orange Money",
        accountName: church.donation_orange_name,
      },
    } as const;

    const selectedMobile =
      channel in mobileConfig
        ? mobileConfig[channel as keyof typeof mobileConfig]
        : null;

    if (
      method === "mobile_money" &&
      (!selectedMobile?.enabled || !selectedMobile.number)
    ) {
      return NextResponse.json(
        {
          error: "Ce canal Mobile Money n’est pas configuré.",
        },
        { status: 400 },
      );
    }
    ...
    if (
      channel === "card" &&
      (!church.donation_card_enabled || !church.donation_card_url)
    ) {
      return NextResponse.json(
        {
    ...
    if (channel === "cash" && church.donation_cash_enabled === false) {
      return NextResponse.json(
        {
    ...
    const confirmationPath = createConfirmationPath(
      request,
      slug,
      reference,
      publicToken,
    ...
    const absoluteConfirmationUrl = new URL(
      confirmationPath,
      request.url,
    ).toString();
    ...
    const instructionSnapshot = {
      ...
      mobileMoney: selectedMobile
        ? {
            operator: selectedMobile.operator,
            number: selectedMobile.number,
            accountName: selectedMobile.accountName,
          }
        : null,
      bank:
        channel === "bank_transfer"
    ...
    const status = channel === "cash" ? "submitted" : "awaiting_payment";
    ...
    const { data: donation, error: insertError } = await admin
      ...
      .insert({
        ...
        external_payment_url:
          channel === "card" ? church.donation_card_url : null,

        payment_instructions_snapshot: instructionSnapshot,
      ...
      .select("id, reference, public_token, status")
      .single();

    if (insertError || !donation) {
      return NextResponse.json(
        {
          error: insertError?.message || "Impossible d’enregistrer ce don.",
        },
        { status: 500 },
      );
    }
    ...
    const paymentUrl =
      channel === "card" && church.donation_card_url
        ? buildPaymentUrl({
            rawUrl: church.donation_card_url,
            amount,
            currency,
            reference,
            returnUrl: absoluteConfirmationUrl,
          })
        : null;
    ...
    try {
      const transaction = await createPendingDonationPayment(
        {
          ...
          provider:
            channel === "card"
              ? church.donation_card_provider_name || "external_card"
              : channel,
      ...
      paymentWarning = paymentErrorMessage(paymentError);
    }
    ...
    return NextResponse.json({
      data: {
        ...
        paymentWarning,
        confirmationUrl: confirmationPath,
        paymentUrl,
        instructions: {
          mobileMoney: selectedMobile
            ? {
