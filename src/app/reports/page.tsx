...
type ReportCard = {
  ...
  href: string;
  icon: LucideIcon;
  tone: "blue" | "green" | "violet" | "amber" | "cyan";
  available: boolean;
  label: string;
};

const toneClasses = {
  blue: "from-blue-700 to-blue-500 shadow-blue-900/20",
  green: "from-emerald-700 to-emerald-500 shadow-emerald-900/20",
  violet: "from-violet-700 to-violet-500 shadow-violet-900/20",
  amber: "from-amber-600 to-orange-500 shadow-orange-900/20",
  cyan: "from-cyan-700 to-sky-500 shadow-cyan-900/20",
};
...
export default async function ReportsPage() {
  ...
  const [
    ...
    canAccessAnyModule(["attendance"], "view"),
    canAccessAnyModule(
      ["finance_reports", "financial_reports", "finance"],
      "view"
    ),
    canAccessAnyModule(["extension_reports", "extensions"], "view"),
    canAccessAnyModule(["donations", "finance"], "view"),
    canAccessAnyModule(
  ...
  // Plafonds fonctionnels : même si une ancienne permission individuelle existe,
  // un secrétaire ne reçoit jamais les rapports financiers/patrimoine/dons et
  // un Responsable D reste limité à son périmètre département + présence.
  const allowPatrimony =
    canViewPatrimony && !isSecretary && !isDepartmentResponsible;
  const allowFinance =
    canViewFinance && !isSecretary && !isDepartmentResponsible;
  const allowDonations =
    canViewDonations && !isSecretary && !isDepartmentResponsible;
  const allowExtensions = canViewExtensions && !isDepartmentResponsible;

  const cards: ReportCard[] = [
    {
      title: "Rapport patrimoine",
      description:
        "Analysez l’inventaire, la valeur estimée, l’état des biens, les maintenances et les mouvements sur la période choisie.",
      href: "/reports/patrimony",
      icon: PackageSearch,
      tone: "violet",
      available: allowPatrimony,
      label: "Patrimoine",
    },
    {
      ...
      description:
        isDepartmentResponsible
          ? "Préparez et transmettez le rapport mensuel de votre département."
          : "Suivez la complétude, la promptitude, visualisez et validez les rapports transmis par les départements.",
    ...
    },
    {
      ...
      icon: CalendarCheck,
      tone: "blue",
      available: canViewAttendance,
      label: "Présences et événements",
    },
    {
  ...
  if (availableCards.length === 0) {
    redirect("/unauthorized?reason=reports_access");
  }
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-7 text-blue-50">
                Retrouvez uniquement les rapports autorisés pour votre fonction,
                consultez les indicateurs et suivez les validations attendues.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-lg font-black text-[#03357A]">
                Accès sécurisé par fonction
              </h2>

              <p className="mt-2 text-sm leading-7 text-slate-600">
                Les rapports affichés respectent à la fois les permissions du
                compte et le périmètre fonctionnel de son rôle.
              </p>
            </div>
          </div>
        </section>
